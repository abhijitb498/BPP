...
double precision :: m(2), m12, z(2), AN(2), R0, rad(2), hbar, amu, R1, R2, Rs, aa, bb, d2r, r2d
...
!R1 = 1.0
!do i=1,500
!write(10,*)R1, f(R1,m12,z,l,Ecm)+Ecm
!R1 = R1 + 0.1
!enddo

aa = 2.0
bb = 20.0 !! As solve uses BISECTION METHOD, you need to provide to initial points, 1st one is rho0, 2nd one is > rho0

call solve(aa,bb,R1,m12,z,l,Ecm)
!write(*,*)R1
...
No = 1000  ! Integration Points for the subroutine simpsint
call simpsint(R1, R2, No, m12, z, l, Ecm, S)
!print*, S
kmin = sqrt(2.0*m12*amu*(Vcoulcf(R1,m12,z,l) - Ecm))/hbar
...
!P = k/(1.0+exp(2.0*S))! Alpha Decay Probability
...
double precision :: f, x, m, z(2), l, Ex, hbar, Vcoul

hbar = 197.326

if (x .lt. Rs) then
!print*, x
Vcoul = (1.44*z(1)*z(2))/(2.0*Rs)*(3.0 - x**2.0/Rs**2.0) 
else
Vcoul = (1.44)*(z(1)*z(2)/x) 
endif

f = Vcoul + ((hbar**2.0)*l*(l+1))/(2.0*m*amu*x**2.0) &
- 1.535/(1+exp((x-5.182)/0.123)) - 4.0*5.397/(1+exp((x-2.584)/0.997))**2.0 - Ex
!f = x**2.0 - 1.0
...
function Vcoulcf(x,m,z,l)
implicit none

double precision :: Vcoulcf, x, m, z(2), l, hbar

hbar = 197.326

if (x .lt. Rs) then
!print*, x
Vcoulcf = (1.44*z(1)*z(2))/(2.0*Rs)*(3.0 - x**2.0/Rs**2.0) + ((hbar**2.0)*l*(l+1))/(2.0*m*amu*x**2.0)
else
Vcoulcf = (1.44)*(z(1)*z(2)/x) + ((hbar**2.0)*l*(l+1))/(2.0*m*amu*x**2.0)
endif
